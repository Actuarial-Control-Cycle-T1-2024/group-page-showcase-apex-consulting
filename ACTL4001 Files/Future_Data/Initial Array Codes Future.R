...
x <- as.data.frame(Policyholders_Future[1,,,,,,])
#---------------------------------------------------------------------------------------------------------
...
reserve_function <- function(age, projection_time, policy, sums_assured) {
  ...
  if (policy == SPWL) {
    ...
    Prospective_Reserve <- sums_assured* Term_Assurance(age+(projection_time-1),length, reserve_rate) +
      100000 /sums_assured * Net_Premium * expense_margin * (1 - ifelse(projection_time >1,
                                                                        Term_Annuity(age, projection_time-1, reserve_rate),
                                                                        0)
                                                             /Term_Annuity(age, MAX_AGE-age, reserve_rate)) - 
      ifelse(projection_time == 1, Net_Premium *(1+expense_margin * 100000/sums_assured), 0)
  } else{
...
Premium <- aperm(Premium, c(1:5,7,6))
#-------------------------------------------------------------------------------------------------------------
...
fwrite(as.data.frame(Reserve_Array), file = "~/ACTL4001/Future_Data/Current_Reserves.csv" )
fwrite(as.data.frame(Reserve_Array_eop), file = "~/ACTL4001/Future_Data/Current_Reserves_eop.csv" )
