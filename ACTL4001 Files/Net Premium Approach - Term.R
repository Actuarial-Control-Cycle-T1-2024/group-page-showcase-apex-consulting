    ...
    for (i in 25:65) {
      ...
      for (l in 1:DISTRIBUTION_CHANNEL) {
        ...
        for (m in 1:SUM_ASSURED) {
          ...
          Premium_Array[i,j,k,l,m] <- Sum_Assured[m]*Term_Assurance(i, 20, discount_rate) / Term_Annuity(i, 20, discount_rate) *
            (1+Profit_Margin + commission_margin[l]+expense_margin * 100000/Sum_Assured[m])
        }
      }
    }
  }
...
fwrite(as.data.frame(Premium_Array), "~/ACTL4001/Initialised_Data/Net_Premium_Term.csv")
