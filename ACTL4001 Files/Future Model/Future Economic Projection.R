...
Reserves <- array(unlist(fread("~/ACTL4001/Future_Data/Current_Reserves.csv")), 
                   dim = c(MAX_AGE, COHORTS, GENDER, SMOKING_TYPES,
                           POLICY_TYPE, SUM_ASSURED, DISTRIBUTION_CHANNEL))


Reserves_eop <- array(unlist(fread("~/ACTL4001/Future_Data/Current_Reserves_eop.csv")), 
                  dim = c(MAX_AGE, COHORTS, GENDER, SMOKING_TYPES,
                          POLICY_TYPE, SUM_ASSURED, DISTRIBUTION_CHANNEL))
...
Investment_Rate <- matrix(unlist(fread("~/ACTL4001/Future_Data/Investment_Data.csv")), ncol = 30)
Inflation_Rate <- matrix(unlist(fread("~/ACTL4001/Initialised_Data/Inflation_Rates.csv")), ncol = 30)
...
Investment_Rate <- apply(Investment_Rate, MARGIN = 2, FUN = mean)
Investment_Rate <- rep(c(0.05),21)


# Investments will maintain at a steady rate (based on diversification of resorces)
# Inflation will return to Pre-Covid Levels

Inflation_Rate <- c(1,1.06, 1.04, rep(1.03, 18))


Inflation_Rate <- cumprod(Inflation_Rate)
...
for (n in 1:NUMBER_OF_SIMULATIONS) {
  ...
  for (j in 1: (PROJECTION_TIME + 1)) {
    ...
    mortality_matrix_IND <- mortality_base * ifelse(INITIATIVE == 1,
                                                    reduction_matrix, 1)
  ...
  for (i in 2:(PROJECTION_TIME+1)) {
    ...
    for (j in 1:COHORTS) {
      ...
      Reserves_at_point_eop[43,35,1,1,2,1,1]
      Reserves_at_point_sop[42,36,1,1,2,1,1]
      if (i+22 == j) {
    ...
    Profit_Simulations[n,i ] <- sum(Profit_Array[,,,,SPWL,,]) 
  }
...
Profit_Simulations[1,]

sum(as.data.frame(Reserves_eop[,,,,T20,,]))

sum(Reserves[26:66,i+23-42,,,T20,,] == Reserves_eop[26:66,i+23-42,,,T20,,])
sum(Reserves[26:66,i+23-42,,,SPWL,,] == Reserves_eop[26:66,i+23-42,,,SPWL,,])

o <- as.data.frame(Premium[,i-j,,,T20,,])

x <- as.data.frame(Premium[,j,,,,,])

Reserve_Amount[1,77,2,1,1,2,1]
Reserve_Amount_eop[1,77,2,1,1,2,1]

l <- as.data.frame(Premium_at_point[43,,,,,,])
Premium_at_point[43,35,1,1,2,1,1]

h <- as.data.frame(Policyholders_Current[43,j,,,,,])

m <- as.data.frame(Premium_Income[43,,,,,,])
m
a <- as.data.frame(Reserve_Amount[43,,,,,,] - Reserve_Amount_eop[43,,,,,,])
a
ax <- as.data.frame(Reserve_Amount[1,,,,,,])

b <- as.data.frame(Reserves_at_point_eop[43,,,,,,])
f <- as.data.frame(Reserves_at_point_eop[42,,,,,,])
m <- as.data.frame(Premium_Income[43,,,,,,])

y <- as.data.frame(Reserves_at_point_sop[43,,,,,,] *(Policyholders_Current[43,,21,,,,,] 
                                                     + Policyholders_Mortality[43,,21,,,,,] 
                                                     + Policyholders_Withdrawl[43,,21,,,,,]))

Profit_Simulations_Intervention <- Profit_Simulations[1:270,] 
...
Profit_Simulations_Intervention2 <- Profit_Simulations_Intervention * matrix(apply(matrix(c(0:20), nrow = 1), 2,function(x){(1+risk_discount_rate)^(x)}), nrow = 270,ncol = 21, byrow= T)
...
Profit_Bandings <- data.frame(time = 0:PROJECTION_TIME,
                              Median = quantiles[3, ],
                              P25 = quantiles[2, ],
                              P75 = quantiles[4, ],
                              P5 = quantiles[1, ],
...
Profit_Simulations_Intervention2 <- Profit_Simulations_Intervention2 +100*10^6

Profit_Simulations[1,]


x <- Profit_Simulations[1,]


Profit_Bandings
Base_set <- fread("~/ACTL4001/Base_Case.csv")



Base_set <- Base_set * apply(matrix(c(0:20), nrow = 1), 2,function(x){(1+risk_discount_rate)^(-x)})

Base_set <- fread("~/ACTL4001/New Base.csv")
Base_set <- Profit_Simulations[1,]


ggplot(Profit_Bandings, aes(x = time, y = mean)) +
  geom_rect(aes(ymin = 0.1, ymax = 0.10001, xmin = 1.05, xmax = 1.05000001, fill = "Baseline"))+
  ...
  geom_line(mapping = aes(x = c(0:20), y =unlist(Base_set)), col = "#420480", lwd=  1.5) +
  labs(title = "Superlife profit projection over 20 years",
       x = "Projection Time",
       y = "Profit (Crowns)") +
  scale_fill_manual(name = "Percentile Range",
                    values = c("Baseline" = "black", "Mean" =  "#420480","25th - 75th Percentiles" = "#129B99", "5th - 95th Percentiles" = "#0676C6"),
                    labels = c("25th - 75th Percentiles", "5th - 95th Percentiles","Mean", "Base")) +
  theme_bw() +
