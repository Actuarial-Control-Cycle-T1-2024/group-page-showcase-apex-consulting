    ...
    for (i in 25:65) {
      ...
      for (l in 1:DISTRIBUTION_CHANNEL) {
        ...
        for (m in 1:SUM_ASSURED) {
          ...
          Premium_Array[i,j,k,l,m] <- ((Sum_Assured[m]+300) * Term_Assurance(i, 20, discount_rate) / Term_Annuity(i, 20, discount_rate) 
          + (150-45)/Term_Annuity(i, 20, discount_rate) + 45) * (1+Profit_Margin)*(1 + commission_margin[l])
        }
      }
    }
  }
