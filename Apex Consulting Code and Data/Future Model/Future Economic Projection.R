...
# Code has been used to create both the base projection model (STOCHASTIC OFF)
# Code can be run with the simulations and initiative turned on
# This will run through a set of simulations of the profit array
...
for (n in 1:NUMBER_OF_SIMULATIONS) {
  ...
  if (n != 1) {
    Inflation <- cumprod(c(1,1+ Inflation_Rate[n,2:(PROJECTION_TIME+1)]))
  } else {
...
fwrite(as.data.frame(Profit_Simulations[1,]), "~/ACTL4001/Simulations Data/Base Case Combined SPWL and T20.csv")
fwrite(as.data.frame(Profit_Simulations_SPWL[1,]), "~/ACTL4001/Simulations Data/Base Case SPWL.csv")
fwrite(as.data.frame(Profit_Simulations_SPWL[1,]), "~/ACTL4001/Simulations Data/Base Case T20.csv")