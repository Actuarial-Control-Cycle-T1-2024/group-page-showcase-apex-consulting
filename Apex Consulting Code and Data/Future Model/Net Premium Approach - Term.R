...
# Code to calculate the premium, using a very simplistic assumption to account for;
# Scope of data
# Efficiency of the program for profit testing
...
expense_margin <- c(0.15)
discount_rate <- 0.04
fixed_initial_expense <- 150
renewal_expense <- 45
mortality_cost <- 300
...
Mortality_Rates$weighted_male_S_qx/Mortality_Rates$weighted_female_NS_qx
    ...
    for (i in 25:65) {
      ...
      for (l in 1:DISTRIBUTION_CHANNEL) {
        ...
        for (m in 1:SUM_ASSURED) {
          ...
          Premium_Array[i,j,k,l,m] <- ((Sum_Assured[m]+Mortality_Cost) * Term_Assurance(i, 20, discount_rate) / Term_Annuity(i, 20, discount_rate) 
          + (fixed_initial_expense-renewal_expense)/Term_Annuity(i, 20, discount_rate) + renewal_expense) * (1+Profit_Margin)*(1 + commission_margin[l])
        }
      }
    }
  }
...
fwrite(as.data.frame(Premium_Array), "~/ACTL4001/Future_Data/Net_Premium_Term.csv")
