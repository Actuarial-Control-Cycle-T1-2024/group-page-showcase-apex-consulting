...
for (j in 2: (PROJECTION_TIME + 1)) {
    ...
    Policyholders_Future[j,,,,T20,j+1,LAPSED] <- Policyholders_Future[j,,,,T20,j+1,NOT_LAPSED]
    
    # Set lives in policy to 0 (at end fo the term)
    
    Policyholders_Future[j,,,,T20,j+1,NOT_LAPSED] <- 0
...
y <- Policyholders_Future[10,,,1,1,1,1]
