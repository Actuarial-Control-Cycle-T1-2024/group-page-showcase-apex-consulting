...
Policyholders_Current[1:3,,1,,,,] <- Policyholders_Joining[1:3,,,,,]

# Withdrawing assumptions for different ages, gender etc. for term insurance

withdraw_matrix_IND <- array(c(0.01), dim = c(MAX_AGE, GENDER, SMOKING_TYPES, COHORTS, SUM_ASSURED))

mortality_matrix_IND <- array(c(0.02), dim = c(MAX_AGE, GENDER, SMOKING_TYPES, POLICY_TYPE, COHORTS, SUM_ASSURED))

# Make the rates dependent under the UDD assumption

mortality_matrix <- array(c(0), dim = c(MAX_AGE, GENDER, SMOKING_TYPES, POLICY_TYPE, COHORTS, SUM_ASSURED))
...
for (j in 2: (PROJECTION_TIME + 1)) {
  ...
  Policyholders_Current[j+2,,j,,,, ] <- Policyholders_Joining[j,,,,,]
  ...
  Policyholders_Current[j,,,,,,] <- (1- mortality_matrix)* Policyholders_Current[j,,,,,,]
  
  # ---- Apply Withdraw ----
  
  # Keep track of participants who are withdrawn
  
  Policyholders_Withdraws[j,,,,T20,,] <- withdraw_matrix* Policyholders_Current[j,,,,T20,,]
  ...
  if(j +2003 - 2020 >= 0) {
    Policyholders_Withdraws[j,,,,T20,j+2003-2020,] <- Policyholders_Current[j,,,,T20,j+2003-2020, ]
...
y <- Policyholders_Current[2,,,,,T20,]

y <- as.data.frame(y)

Save_1 <- Policyholders_Deaths
...
Results <- (Save_2 - Save_1) * aperm(Face_Values, c(2,3,4,5,6,7,1))

year_of_projection <- apply(Results, MARGIN = c(3), FUN = sum)


plot(c(0:20), year_of_projection)
