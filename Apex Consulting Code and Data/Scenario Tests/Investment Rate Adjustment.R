...
# This script is just tweaking the investment rate scenario
...
Investment_Rate <- t(apply(matrix(seq(0.045, 0.055, by = 0.0025)), 1, function(x){rep(x,21)}))
...
for (n in 1:NUMBER_OF_SIMULATIONS) {
  ...
  for (i in 2:(PROJECTION_TIME+1)) {
    ...
    Interest <- ( Net_Inflow + Reserve_Amount) * Investment_Rate[n,i]
...
ggplot() +
  geom_rect(aes(ymin = 0.1, ymax = 0.10001, xmin = 1.05, xmax = 1.05000001, fill = "Baseline Investment Rate (5%)"))+
  geom_rect(aes(ymin = 0.1, ymax = 0.10001, xmin = 1.05, xmax = 1.05000001, fill = "+-0.25% Investment"))+
  geom_rect(aes(ymin = 0.1, ymax = 0.10001, xmin = 1.05, xmax = 1.05000001, fill = "+-0.5% Investment"))+
  ...
  labs(title = "Sensitivity of baseline investment assumptions",
       x = "Projection Time",
       y = "Profit (Crowns)") +
  scale_fill_manual(name = "Percentile Range",
                    values = c("Baseline Investment Rate (5%)" =  "black",
                               "+-0.25% Investment" = "#129B99", "+-0.5% Investment" = "#0676C6"),
                    labels = c("+-0.25% Investment", "+-0.5% Investment","Baseline Investment Rate (5%)")) +
  theme_bw() +
