...
# This script adjusts the base model with the expense rate
...
# Initial Expenses
Fixed_Initial_Cost_base <- array(unlist(fread("~/ACTL4001/Future_Data/Fixed_Initial_Cost.csv")),
                            dim = c(COHORTS, MAX_AGE, GENDER, SMOKING_TYPES,
                                    POLICY_TYPE, SUM_ASSURED, DISTRIBUTION_CHANNEL))


Yearly_Cost_base <- array(unlist(fread("~/ACTL4001/Future_Data/Yearly_Cost.csv")),
                     dim = c(COHORTS, MAX_AGE, GENDER, SMOKING_TYPES,
                             POLICY_TYPE, SUM_ASSURED, DISTRIBUTION_CHANNEL)) 

Mortality_Cost_base <- array(unlist(fread("~/ACTL4001/Future_Data/Mortality_Cost.csv")),
                        dim = c(COHORTS, MAX_AGE, GENDER, SMOKING_TYPES,
                                POLICY_TYPE, SUM_ASSURED, DISTRIBUTION_CHANNEL))
...
Expense_Adjustment <- seq(0, 2, by = 0.5)
...
for (n in 1:NUMBER_OF_SIMULATIONS) {
  ...
  Fixed_Initial_Cost <- Fixed_Initial_Cost_base * Expense_Adjustment[n]
  Yearly_Cost <- Yearly_Cost_base * Expense_Adjustment[n]
  Mortality_Cost <- Mortality_Cost_base * Expense_Adjustment[n]  
...
ggplot() +
  ...
  geom_rect(aes(ymin = 0.1, ymax = 0.10001, xmin = 1.05, xmax = 1.05000001, fill = "+- 50% Expense Adjustment"))+
  geom_rect(aes(ymin = 0.1, ymax = 0.10001, xmin = 1.05, xmax = 1.05000001, fill = "+- 100% Expense Adjustment"))+
  ...
  labs(title = "Sensitivity of baseline expense assumptions",
       x = "Projection Time",
       y = "Profit (Crowns)") +
  scale_fill_manual(name = "Percentile Range",
                    values = c("Baseline Model" =  "black",
                               "+- 50% Expense Adjustment" = "#129B99", "+- 100% Expense Adjustment" = "#0676C6"),
                    labels = c("+-100% Expense Adjustment", "+-50% Expense Adjustment","Baseline Model")) +
  theme_bw() +
