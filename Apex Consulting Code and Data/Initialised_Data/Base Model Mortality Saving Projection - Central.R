...
# Code has been built to create past mortality savings
# This is done for pure mortality reduction and 
# reduction in the cost assumptions
...
# Mortality_Reduce <- array(unlist(fread("~/ACTL4001/Future_Data/Mortality_Reducs-NonSmokes.csv")), dim =c(120,10000,25))[,7500:10000,]
# Mortality_Reduce_smokers <- array(unlist(fread("~/ACTL4001/Future_Data/Mortality_Reducs-Smokes.csv")), dim =c(120,10000,25))[,7500:10000,]
# 
# Cost_Reduce <- array(unlist(fread("~/ACTL4001/Future_Data/Cost_Amounts-NonSmokes.csv")), dim = c(120, 10000, 25))[,7500:10000,] 
# Cost_Reduce_smokers <- array(unlist(fread("~/ACTL4001/Future_Data/Cost_Amounts-Smokes.csv")), dim = c(120, 10000, 25))[,7500:10000,]
...
for (n in 1:NUMBER_OF_SIMULATIONS) {
  ...
  Inflation <- cumprod(rev(c(1, Inflation_Rate[2:(PROJECTION_TIME+1)])))
  Inflation <- rev(Inflation)
  ...
  for (j in 1: (PROJECTION_TIME + 1)) {
    ...
    #Mortality_Reduce[,n,j], Mortality_Reduce_smokers[,n,j]
    ...
    # Apply Adjustment for COVID Mortality levels
    
    mortality_matrix_IND <- mortality_base
    
    mortality_matrix_IND[,,,,,,,1:17] <- mortality_base[,,,,,,,1:17]#*(1-reduction_matrix[,,,,,,,1:17])
    
    mortality_matrix_IND[,,,,,,,18:21] <- 1-(1-mortality_base[,,,,,,,18:21])^(1.19) #-  mortality_base[,,,,,,,18:21] * reduction_matrix[,,,,,,,18:21]
  ...
  Inflation_Assumption <-array(c(Inflation), dim = c(PROJECTION_TIME+1, MAX_AGE, SMOKING_TYPES,COHORTS,
                                  GENDER, POLICY_TYPE,SUM_ASSURED, DISTRIBUTION_CHANNEL))
  
  Inflation_Assumption <- aperm(Inflation_Assumption, c(2:5,1,6:8))
  Inflation_Assumption <- aperm(Inflation_Assumption,c(3,1,5,4,2,6:8) )
  ...
  Profit_Array <- Death_Costs[,,2:(PROJECTION_TIME+1),,,,,] #+ Initiative_Costs[,,(2:(PROJECTION_TIME+1)),,,,,]* 1/Inflation_Assumption[,,(2:(PROJECTION_TIME+1)),,,,,]
...
base_cost <- apply(Profit_Array, 3, sum) 

#-----------------------------------------------------------------------------------------------------

post_death <- apply(Death_Count, 3, sum)
post_cost <- apply(Profit_Array, 3, sum)
...
# Plot
ggplot() +
  geom_rect(aes(ymin = 0.1, ymax = 0.10001, xmin = 2005.01, xmax = 2005.02, fill = "Expected Pre Initiative"))+
  geom_rect(aes(ymin = 0.1, ymax = 0.10001, xmin = 2005.01, xmax = 2005.02, fill = "Expected Post Initiative"))+
  ...
  geom_line(mapping = aes(x = c(2004:2023), y = base_deaths), col = "#044B7F", lty ="dashed", lwd = 1.5) +
  ...
  scale_fill_manual(name = "Scenario",
                    values = c("Expected Pre Initiative" = "#044B7F", "Expected Post Initiative" = "#25A0F8"),
                    labels = c("Expected Post Initiaitive", "Expected Pre Initiative","Mean")) + 
  theme_bw() +
...
ggplot() +
  ...
  geom_line(mapping = aes(x = c(2004:2023), y = post_cost), col = "#25A0F8", lwd = 1.5) +
  labs(title = "Superlife historical mortality cost savings",
       x = "Projection Time",
       y = "Mortality Costs") +
  scale_fill_manual(name = "Scenario",values = c("Expected Pre Initiative" = "#044B7F", "Expected Post Initiative" = "#25A0F8"),
                    labels = c("Expected Post Initiaitive", "Expected Pre Initiative","Mean")) +
  theme_bw() +
  theme(legend.position = "bottom",
        legend.justification = "center",
  ...
  guides(fill = guide_legend(reverse = TRUE)) +
  scale_x_continuous("Projection Time", expand = c(0,0)) +
  scale_y_continuous("Expected Mortality Costs", labels = scales::unit_format(unit = "B", scale = 1e-9))

sum(base_cost- post_cost)
post_death-base_deaths
apply(Deaths, 3, sum)[2:(PROJECTION_TIME+1)] - base_deaths
